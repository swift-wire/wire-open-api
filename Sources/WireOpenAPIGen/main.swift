...
// One conformer per spec, for now. Dispatching per operation removes the reason this was ever a
// constraint — several controllers could each contribute the operations they declare to one conformer,
// with the compiler checking the set is complete — but the conformer currently holds a single subject.
// Until it holds several this is a hard error rather than a silently half-served document.
for (spec, controllers) in byGroup where controllers.count > 1 {
    ...
    FileHandle.standardError.write(
        Data(
            ...
            \(label) (\(named)). One conformer serves a document, and it holds one subject. Give them \
            distinct `spec:` values, or wait for multi-subject conformers.
...
for (spec, controllers) in byGroup.sorted(by: { $0.key < $1.key }) {
    ...
    // Controller-scope `@Middleware`, folded around every operation of this spec. The fold is emitted at
    // each registration rather than run from a helper because `wireCompose` returns the chain's concrete
    // composed type — erasing it would stop the terminal reading the final box's contents.
    //
    ...
    DirectDispatchEmitter(
        ...
        scoped: controllers.first(where: { $0.seed != nil }),
