...
for (spec, controllers) in byGroup.sorted(by: { $0.key < $1.key }) {
    ...
    // field, so the witness delegates to the controller's own generated registration. No base path is
    // passed: the prefix belongs to the document's `servers:` block, which `.defaultOpenAPIServerURL`
    // already reads.
    lines.append(
        """
