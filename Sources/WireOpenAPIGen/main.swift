...
// `Wire` only when a conformer emits the `noSubject` helper, which is the sole reference to anything in
// that module (`WireScopeEntry`, to project a subject type out of a scope-entry thunk). Importing it
// unconditionally would be unused in almost every output, and the compiler warns on that.
let wireImport = needsScopeEntryProjection(discovered) ? ["Wire"] : []
let allImports = Set(["Foundation", "OpenAPIRuntime", "WireMVC", "WireOpenAPI"] + wireImport + imports)
    .sorted()
