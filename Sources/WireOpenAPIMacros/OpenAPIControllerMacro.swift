...
/// that proxy's `TransportContributor` conformance.
///
/// It used to expand to `extension <Controller>: TransportContributor`. That cannot survive aggregation:
/// with one conformer per spec the witness belongs to the proxy, not to any single controller — and it
/// could not be emitted from generated code anyway, since `TransportContributor` refines `Sendable` and
