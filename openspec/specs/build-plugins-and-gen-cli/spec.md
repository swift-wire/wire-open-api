...
named `""` when none has been given. The first source group SHALL be the compiling module: that is
the first `--module`, unless a source precedes every `--module`, in which case it is the `""` group.
With no arguments it SHALL write `WireOpenAPIGen: missing output path` to standard error and exit with
status 1.
...
- **THEN** `Gate` is the compiling module and its controllers are compiled against `openapi.yaml`
...
### Requirement: A repeated document, config or settings flag replaces the earlier one
Each of `--spec`, `--spec-config` and `--spec-settings`, and each of `--spec-module`,
`--spec-module-config` and `--spec-module-settings` for a given module, SHALL hold one path in
`WireOpenAPIGen`: a repeat SHALL replace the earlier value without a diagnostic, so the last one wins.
Because the plugins pass one `--spec` or `--spec-module <M>` per matching file, a module with more
than one such file is read from its last one only, which is tracked as a possible defect in
https://github.com/swift-wire/wire-open-api/issues/82.

#### Scenario: two documents for the compiling module
- **WHEN** WireOpenAPIGen runs as `WireOpenAPIGen out.swift --spec a.yaml --spec b.yaml --module Gate Controller.swift`
- **THEN** `Gate`'s controllers are compiled against `b.yaml` only, and nothing reports that `a.yaml` was ignored
...
Pinned by: `.github/workflows/build.yml` (`Fixtures` job, step `Serve an OpenAPI operation and a @Get route from one router`, the `badResponse` assertion), for the `OrdersAPI` half only. That the app's Tasks operations are unchecked is pinned by nothing yet: no probed Tasks response breaks the app's document, which is tracked in https://github.com/swift-wire/wire-open-api/issues/85.
...
- **WHEN** WireOpenAPIGen is run over the `route-scope/status-not-declared` case and exits with status 1
- **THEN** no `out.swift` is written in its working directory

Pinned by: nothing yet. `DiagnosticGoldenTool` records only the exit status and standard error of each case and never inspects `out.swift`, and nothing asserts the header line.
...
there; and record `accepted` for exit status 0 with empty standard error, `accepted, with output:`
followed by the standard-error text for exit status 0 with some, or `rejected: <stderr>` otherwise,
with standard error trimmed of surrounding whitespace, under a `## <case name>` heading and a
`# <summary>` line.
...
`WireOpenAPIGen no longer answers diagnostics-golden.txt as recorded.` with a `was:`/`now:` pair for
each line of the rebuilt table that differs from the golden line at the same index, and exit 1.
Golden lines beyond the rebuilt table's length are not listed, which is tracked in
https://github.com/swift-wire/wire-open-api/issues/84. On a match it SHALL print
`diagnostics match the golden (<N> cases).`. Without `--check` it SHALL rewrite the file.
...
`.build/release`, `Fixtures/.build/out/Products/Debug` and `Fixtures/.build/debug`. When
`WIRE_OPENAPI_GEN_PATH` is unset and none of the candidates exists it SHALL fail with a message
beginning `cannot find a built WireOpenAPIGen. Build it first:`. When the binary it found among the
candidates has an earlier modification time than any `.swift` file under `Sources/WireOpenAPIGen` it
SHALL fail with a message stating the binary `is older than Sources/WireOpenAPIGen` and naming
`swift build --product WireOpenAPIGen`. A binary named by `WIRE_OPENAPI_GEN_PATH` SHALL be used as
given, without either check, which is tracked as a possible defect in
https://github.com/swift-wire/wire-open-api/issues/83.

#### Scenario: a diagnostic edited without rebuilding
- **WHEN** `Sources/WireOpenAPIGen/Diagnostics.swift` is saved after the last `swift build` and `swift run DiagnosticGoldenTool` is run with `WIRE_OPENAPI_GEN_PATH` unset
